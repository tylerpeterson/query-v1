...
ManualAuthApp.prototype.refreshTokenPromise = function() {
  ...
  if (!this.refreshTokenDfd) {
    ...
    readFile(path.join(this.cacheDirectory, '.tokens')).then(function (fileData) {
      var tokenData = JSON.parse(fileData);
      that.refreshTokenDfd.resolve(tokenData.refreshToken);
    }, function (err) {
      if (err.code === 'ENOENT') {
        // TODO generate a new token via oauth if necessary
        // hmm... generating a new token via oauth is different for command line tools
        // than it is for web apps. If a command line tool then launch a browser
        // pointed at the start of the oauth flow. If an app, then redirect to start of
        // the oauth flow.  Since I intend to do both it looks like I should use a
        // Strategy to configure which thing happens.
      }
      console.log(err);
      that.refreshTokenDfd.reject(err);
    });
